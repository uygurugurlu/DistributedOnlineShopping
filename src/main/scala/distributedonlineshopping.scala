...
/*    val rdd = df.rdd
  ...
  .map(_.swap)*/
    ...
    val graph: Graph[(String,Int, Int), Int] = Graph(vertexRDD, edgeRDD)
...
/*    val shortestPath = result               // result is a graph
  ...
  def getSourceVertex(vid: Long, graph: Graph[(String,Int, Int), Int]): VertexId ={
    val sourceVertex=graph.vertices.filter { case (id,(_,_,_)) => id == vid}.first
    val res=sourceVertex._1
    println(res)
    return res
  }
  def getShortestPath(vertexId1: VertexId, vertexId2: VertexId, graph: Graph[(String,Int, Int), Int]): Option[Int] = {
    val result = ShortestPaths.run(graph, Seq(vertexId1, 2L))
